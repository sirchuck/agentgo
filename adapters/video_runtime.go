...
func ExecuteVideo(ctx context.Context, model ModelConfig, req VideoRequest) (VideoResult, error) {
	...
	case "kling_video":
		return executeConfiguredVideo(ctx, model, req, videoAdapterDefaults{
			...
			modelFieldPath:     "model_name",
		})
	case "sora_video":
		return executeSoraVideo(ctx, model, req)
	case "xai_video":
		return executeXAIVideo(ctx, model, req)
...
func executeXAIVideo(ctx context.Context, model ModelConfig, req VideoRequest) (VideoResult, error) {
	prepared, err := prepareXAIModel(model)
	if err != nil {
		return VideoResult{}, err
	}
	if strings.TrimSpace(prepared.APIPath) == "" {
		prepared.APIPath = "/v1/videos/generations"
	}
	endpoint := strings.TrimSpace(modelEndpoint(prepared))
	if endpoint == "" {
		return VideoResult{}, errors.New("missing xAI video submit endpoint")
	}
	prompt := xaiPrompt(Request{Instructions: req.Prompt, Messages: []Message{{Role: "user", Text: req.Prompt}}}, prepared)
	if strings.TrimSpace(prompt) == "" {
		prompt = strings.TrimSpace(req.Prompt)
	}
	if prompt == "" {
		return VideoResult{}, errors.New("xAI video requests require a non-empty prompt")
	}
	payload := map[string]any{}
	for key, value := range providerOptionMap(prepared, "extra_body") {
		if strings.TrimSpace(key) == "" || value == nil {
			continue
		}
		payload[key] = value
	...
	modelName := strings.TrimSpace(prepared.ModelName)
	if modelName == "" {
		modelName = providerOptionString(prepared, "video_model_name", "grok-imagine-video")
	}
	payload[providerOptionString(prepared, "video_model_path", "model")] = modelName
	payload[providerOptionString(prepared, "video_prompt_path", "prompt")] = prompt
	referenceImages := make([]VideoBinary, 0, len(req.ReferenceImages))
	for _, image := range req.ReferenceImages {
		if len(image.Data) > 0 {
			referenceImages = append(referenceImages, image)
		}
	}
	if len(referenceImages) > 0 && req.StartFrame != nil && len(req.StartFrame.Data) > 0 {
		return VideoResult{}, errors.New("xAI reference-to-video cannot be combined with starter-image video jobs")
	}
	if len(referenceImages) > 0 {
		if !isXAIGrokReferenceVideoModel(modelName) {
			return VideoResult{}, errors.New("xAI reference images require model grok-imagine-video")
		}
		if len(referenceImages) > 3 {
			return VideoResult{}, errors.New("AgentGO currently supports up to 3 xAI video reference images")
		}
		encodedRefs := make([]map[string]any, 0, len(referenceImages))
		for _, image := range referenceImages {
			encodedRefs = append(encodedRefs, map[string]any{
				"url": encodedBinaryData("data_url", defaultVideoMIME(image.Name, image.MIMEType), image.Data),
			})
		}
		setNestedField(payload, providerOptionString(prepared, "video_reference_images_path", "reference_images"), encodedRefs)
	}
	if req.StartFrame != nil && len(req.StartFrame.Data) > 0 {
		setNestedField(payload, providerOptionString(prepared, "video_start_frame_path", "image.url"), encodedBinaryData("data_url", defaultVideoMIME(req.StartFrame.Name, req.StartFrame.MIMEType), req.StartFrame.Data))
		if imageTypePath := strings.TrimSpace(providerOptionString(prepared, "video_start_frame_type_path", "")); imageTypePath != "" {
			setNestedField(payload, imageTypePath, providerOptionString(prepared, "video_start_frame_type", "image_url"))
		}
	}
	if seconds, ok := normalizeXAIDuration(req.Duration); ok {
		setNestedField(payload, providerOptionString(prepared, "video_duration_path", "duration"), seconds)
	}
	if strings.TrimSpace(req.AspectRatio) != "" {
		setNestedField(payload, providerOptionString(prepared, "video_aspect_ratio_path", "aspect_ratio"), strings.TrimSpace(req.AspectRatio))
	}
	if strings.TrimSpace(req.Resolution) != "" {
		setNestedField(payload, providerOptionString(prepared, "video_resolution_path", "resolution"), strings.TrimSpace(req.Resolution))
	}
	if strings.TrimSpace(req.OutputFormat) != "" {
		setNestedField(payload, providerOptionString(prepared, "video_output_format_path", "response_format"), strings.TrimSpace(req.OutputFormat))
	}
	...
	result := VideoResult{SubmitRequestURL: endpoint, SubmitRequestBody: string(body), SubmitRawBody: string(respBody), RawBody: string(respBody)}
	if statusCode >= 300 {
		return result, fmt.Errorf("video submit returned %s: %s", status, strings.TrimSpace(string(respBody)))
	}
	var submitPayload any
	if err := json.Unmarshal(respBody, &submitPayload); err != nil {
		return result, fmt.Errorf("video submit response was not valid JSON")
	}
	jobID := strings.TrimSpace(asString(lookupPath(submitPayload, providerOptionString(prepared, "video_job_id_path", "request_id"))))
	if jobID == "" {
		jobID = strings.TrimSpace(asString(lookupPath(submitPayload, "id")))
	}
	if jobID == "" {
		return result, errors.New("video submit response did not include a request id")
	}
	...
	pollTemplate := providerOptionString(prepared, "video_poll_path_template", "/v1/videos/{job_id}")
	statusPath := providerOptionString(prepared, "video_status_path", "status")
	successValue := strings.ToLower(strings.TrimSpace(providerOptionString(prepared, "video_success_value", "done")))
	resultURLPath := providerOptionString(prepared, "video_result_url_path", "video.url")
	resultMimePath := providerOptionString(prepared, "video_result_mime_path", "video.mime_type")
	for {
		if ctx.Err() != nil {
			result.Status = "stopped"
			return result, ctx.Err()
		}
		pollURL := buildVideoPollURL(prepared, jobID, pollTemplate)
		pollReq, err := http.NewRequestWithContext(ctx, http.MethodGet, pollURL, nil)
		if err != nil {
			return result, err
		}
		...
		result.PollRequestURL = pollURL
		result.LastPollRawBody = string(pollBody)
		result.RawBody = string(pollBody)
		result.PollCount++
		result.LastPollAt = time.Now().UTC().Format(time.RFC3339)
		var pollPayload any
		if pollCode < 300 {
			_ = json.Unmarshal(pollBody, &pollPayload)
		}
		remoteStatus := strings.ToLower(strings.TrimSpace(asString(lookupPath(pollPayload, statusPath))))
		result.RemoteStatus = remoteStatus
		if req.PollUpdate != nil {
			req.PollUpdate(VideoProgress{Status: result.Status, ProviderJobID: result.ProviderJobID, RemoteStatus: result.RemoteStatus, PollCount: result.PollCount, LastPollAt: result.LastPollAt, SubmitRequestURL: result.SubmitRequestURL, SubmitRequestBody: result.SubmitRequestBody, SubmitRawBody: result.SubmitRawBody, PollRequestURL: result.PollRequestURL, LastPollRawBody: result.LastPollRawBody})
		}
		if pollCode >= 300 {
			return result, fmt.Errorf("video polling returned %s: %s", pollStatus, strings.TrimSpace(string(pollBody)))
		}
		switch remoteStatus {
		case successValue:
			videoURL := strings.TrimSpace(asString(lookupPath(pollPayload, resultURLPath)))
			mimeType := strings.TrimSpace(asString(lookupPath(pollPayload, resultMimePath)))
			result.VideoSourceURI = videoURL
			if videoURL == "" {
				return result, errors.New("video poll response did not include a result URL")
			}
			downloaded, err := downloadResponseFileURL(videoURL, modelTimeout(prepared, 20*time.Minute))
			if err != nil {
				return result, err
			}
			result.Status = "completed"
			result.VideoData = downloaded.FileData
			result.VideoFilename = defaultVideoFilename(downloaded.FileName, ".mp4")
			result.VideoMIMEType = defaultVideoMIME(result.VideoFilename, defaultString(mimeType, downloaded.FileMIMEType))
			return result, nil
		case "failed", "expired", "error":
			result.Status = remoteStatus
			message := strings.TrimSpace(asString(lookupPath(pollPayload, providerOptionString(prepared, "video_error_path", "error.message"))))
			if message == "" {
				message = strings.TrimSpace(asString(lookupPath(pollPayload, "error")))
			}
			if message == "" {
