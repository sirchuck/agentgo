...
func selectAdapter(model ModelConfig) (Adapter, error) {
	...
	case "openai_compatible", "xai_responses":
		return openAICompatibleAdapter{}, nil
	case "xai_image":
		return xaiImagineImageAdapter{}, nil
	case "custom_json":
		return customJSONAdapter{}, nil
	case "custom_multipart":
		return customMultipartAdapter{}, nil
	case "veo_video", "vertex_veo_video", "kling_video", "sora_video", "comfyui_ltx_video", "xai_video":
...
func knownModelCapabilityHints(model ModelConfig) ModelCapabilities {
	...
	if adapter == "xai_responses" {
		if containsAny(identity, "grok-4", "grok build", "grok-build") {
			hints.SupportsImageIn = true
		}
	}
	return hints
...
func normalizedAdapterName(model ModelConfig) string {
	...
	case "openrouter", "custom", "openai_compatible", "deepseek", "mistral":
